...
fn emit_actor(actor: &ActorDecl, model: &Model<'_>) -> Result<String> {
    ...
    let emitted_entries = actor
        ...
        .map(|(entry, input_references)| {
            ...
            let uses = emit_entry(&mut out, actor, entry, model, input_references, &state_values)?;
            Ok((out, uses))
        })
        .collect::<Result<Vec<_>>>()?;
    let digest_helpers = emitted_entries.iter().flat_map(|(_, uses)| uses.digest_helpers.iter().cloned()).collect::<BTreeSet<_>>();
    let body_imported_templates =
        emitted_entries.iter().flat_map(|(_, uses)| uses.imported_templates.iter().cloned()).collect::<BTreeSet<_>>();
    let mut out = String::new();
    ...
    let imported_templates = imported_template_specs_for_actor(actor, model)
        .into_iter()
        .chain(body_imported_templates)
        .collect::<BTreeSet<_>>()
        .into_iter()
        .collect::<Vec<_>>();
    emit_imported_template_constants(&mut out, &imported_templates);
...
fn emit_entry(
    ...
    out.push_str("    }\n");
    Ok(EntryCodegenUses { digest_helpers: lowered_body.digest_helpers, imported_templates: lowered_body.imported_templates })
}

struct EntryCodegenUses {
    digest_helpers: BTreeSet<SourceStateId>,
    imported_templates: BTreeSet<ImportedTemplateSpec>,
}
...
pub(super) fn hidden_imported_template_const_name(spec: &ImportedTemplateSpec) -> String {
