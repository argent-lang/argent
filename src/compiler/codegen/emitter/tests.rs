...
#[test]
fn rejects_generated_state_type_in_argent_source() {
    for ty in ["State", "State[]", "State[2]", "actor_type<State>"] {
        let err = parse_and_validate(&format!(
            r#"
                ...
                actor Foo owns FooState {{
                    entry inspect({ty} value) emits none {{
                        require(1 == 1);
                    }}
                }}
        ...
        .expect_err("generated physical state type must be reserved in Argent source");

        assert!(err.to_string().contains("uses reserved state type `State`"), "unexpected error for `{ty}`: {err}");
    }
    ...
    let err = parse_and_validate(
        r#"
            ...
            actor Foo owns FooState {
                entry invalid() emits none {
                    State physical = { count: count };
                    require(physical.count >= 0);
                }
            }
    ...
    .expect_err("generated physical state type must be reserved in entry bodies");
    assert!(err.to_string().contains("uses reserved state type `State`"), "unexpected error: {err}");
    ...
    .expect_err("generated physical state type must be reserved in helper bodies");
    assert!(err.to_string().contains("uses reserved state type `State`"), "unexpected error: {err}");
}
...
#[test]
fn current_state_array_entry_param_uses_source_state_type() {
